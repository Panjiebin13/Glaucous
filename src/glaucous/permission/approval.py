...
class ApprovalPipeline:
    ...
    def __init__(self, state: SessionState, callback: DecisionCallback | None, audit: "AuditLog"):
        self._state = state
    ...
    def gate(self, action: ApprovalAction) -> ApprovalVerdict:
        ...
        # 守卫优先级：DANGEROUS 与区外读不可被 auto-approve / 同类型批量豁免
        always_guard = action.risk == Risk.DANGEROUS or (
            action.risk == Risk.WRITE and action.kind == "file_read"
        )
