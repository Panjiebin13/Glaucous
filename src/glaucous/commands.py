"""斜杠命令处理器：分派协议 + 命令注册表（任务 3.3，FR-31，概设 §8）。

v1.1 评审重构：自 1208 行巨石文件拆分——本模块保留分派协议、命令元数据
（COMMAND_META 单一数据源）、ReplContext 聚合与基础命令；命令组按域拆出：
- commands_sessions.py：会话管理（/sessions /rename /fork /stats /resume）；
- commands_rollback.py：/rollback 与 /context；
- commands_spec.py：/spec 与 /specs。
...
async def _cmd_init(ctx: ReplContext) -> bool:
    from .ui.interact import sanitize_input
...
async def _cmd_expand(ctx: ReplContext) -> bool:
    ...
    """
    from .ui.render_events import render_event
...
# 会话管理 / 回退 / Spec 命令组（v1.1 评审拆分；依赖经 TYPE_CHECKING +
# 函数内延迟导入，无模块环）。放在 ReplContext 定义之后导入——
# commands_sessions/_rollback/_spec 仅 TYPE_CHECKING 引用本模块，安全。
from .commands_sessions import (  # noqa: E402, F401
    _cmd_fork,
    _cmd_rename,
    _cmd_resume,
    _cmd_sessions,
...
from .commands_rollback import _cmd_context, _cmd_rollback  # noqa: E402, F401

# Spec 命令组（文件末尾 re-export：测试经 glaucous.commands._cmd_spec 导入）
from .commands_spec import _cmd_spec, _cmd_specs, _spec_status_lines  # noqa: E402, F401
