...
- 折叠只改变**呈现**，不改变事件语义：审计、预算、历史消息均不受影响；
- 摘要行的 N 与 /expand 打印条数一致。
...
- 流式期间保持逐字保真（现状不变）；`stream_end` 处理后追加渲染完整回答为 Markdown 卡片：
...
| R7 | src/glaucous/theme.py、src/glaucous/cli.py | render_answer_card + stream_end 接线 |
