...
- 版本：v1.1 完整版（2026-09-01 增补 §14/§15；2026-09-02 §0/§6 对齐「两大能力 + 3+2 基座」与「三级压缩」对外口径）；覆盖 v1.0 原型到 v1.1 全部里程碑的最终形态
...
- 代码规模：15000+ 行（实现 10547 行 + 测试 4775 行）；38 个测试文件、336 个测试；零 agent 框架
...
**宗旨：高自主地跑长任务，尽可能少打扰用户。**

**两大能力：长任务（跑得动）× 高自主（敢自动）**，由 3+2 五座基座支撑（对外材料亦称「2+3 基座体系」）：

**长任务三座**：
1. **上下文工程**（§6）：三级压缩（L1 本地裁剪 → L2 模型摘要+锚重建 → 预算优雅终止）+ 锚机制（§5.2/§12）——容量不爆、目标不丢；
2. **子 agent 隔离**（§10）：评审等重活在独立上下文执行，主上下文只收千字内结构化报告——容量省着用；
3. **Spec 任务分解**（§12）：澄清 → 书面契约 → 双评审 → 逐任务受管分发 → 验收，防偏移四道闸 + 断点续跑——方向不偏、中断能续。

**高自主两座**：
4. **权限矩阵 + 默认 Build**（§8/§9）：核心判据「有无兜底」——敢放，且默认就放；
5. **Checkpoint 回退**（§11）：每任务入口自动 Git 快照，改坏一条命令还原——输得起。

> 口径沿革：v1.0 文档的「三条主线」（①长任务跑得动 ②需求先说清 ③安全有兜底）在 v1.1 收敛进五基座：「需求先说清」并入基座③的澄清与书面契约机制；「安全有兜底」并入基座④⑤及 §18 协议合法性保障。对外材料（README / 视频 / 答辩）统一使用「两大能力 + 五基座」口径。
...
长任务的命脉。对外口径为「**三级压缩机制**」（L1 → L2 → 预算终止）；**L0 为输出管理首道防线**（单条超长输出截断落盘，不计入压缩级数）。四层按成本与侵入性递增排列：
